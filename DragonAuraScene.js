...
// 龍之光環：擊敗 Boss 後跳出的二選一彈窗。
// 接受：生命上限與攻擊力永久 x2，並在玩家身上留下持續跟隨的金色光環視覺。
// 拒絕：不套用任何加成，直接關閉彈窗繼續遊戲。
export default class DragonAuraScene extends Phaser.Scene {
  constructor() { super('DragonAuraScene'); }
  ...
  create() {
    ...
    this.add.text(w / 2, h * 0.14 + 66, '是否繼承牠的力量，獲得「龍之光環」？', textStyle({
    ...
    this._makeCard(startX, cy, cardW, cardH, {
      icon: 'boss_main', iconTint: 0xffe066, iconScale: 1.3,
      title: '⭐ 接受龍之光環',
      titleColor: '#ffe066',
      desc: '生命上限與攻擊力\n永久提升 2 倍！\n身上會持續纏繞金色龍息氣場',
      onPick: () => this._accept(),
  ...
  _makeCard(cx, cy, cardW, cardH, opt) {
    ...
    this.add.text(cx, cy - 30, opt.title, textStyle({
      fontSize: '34px', color: opt.titleColor, align: 'center',
  ...
  _accept() {
    this.gs.player.applyDragonAura();
    this.gs.enableDragonAuraVisual();
    this.gs.resumeFromDragonAura();
    this.scene.stop();
  }
