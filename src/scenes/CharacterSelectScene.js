...
export default class CharacterSelectScene extends Phaser.Scene {
  ...
  async create() {
    ...
    ids.forEach((char, i) => {
      ...
      const card = this.add.image(cx, cy, 'ui_card').setDisplaySize(cardW, 300).setInteractive({ useHandCursor: true });
      this.add.image(cx, cy - 85, char.texture).setScale(2.8);

      this.add.text(cx, cy - 20, char.typeLabel, {
        fontSize: '12px', color: '#9fd3ff',
      }).setOrigin(0.5);
      this.add.text(cx, cy + 2, char.name, {
        fontSize: '22px', color: '#fff', fontStyle: 'bold',
      }).setOrigin(0.5);
      this.add.text(cx, cy + 30, char.title === '無' ? '' : `「${char.title}」`, {
        fontSize: '13px', color: '#ffd93d', fontStyle: 'italic',
      }).setOrigin(0.5);
      this.add.text(cx, cy + 80, char.desc, {
        fontSize: '12px', color: '#9fd3ff', align: 'center', wordWrap: { width: 220 },
      }).setOrigin(0.5);
    ...
    this.add.text(w / 2, h - 60, '操作：WASD 移動／自動鎖定攻擊／SPACE 衝刺／ESC 暫停', {
