...
const TIME_RING_RADIUS = 92; // 時光戒範圍，跟血肉風暴（knife_sawblade 融合武器）外圈半徑一致
const TIME_RING_ATK_INTERVAL = 1000; // 時光戒範圍內的接觸攻擊間隔（平時 500ms，這裡直接翻倍＝攻速 -50%）
...
export default class EnemySystem {
  ...
  _resetEnemy(sprite, typeId, x, y, tier = 'normal') {
    ...
    sprite.setData('timeRingUntil', 0); // 時光戒：獨立於 slowUntil 的減速旗標，見 update()
  ...
  update(time, delta) {
    ...
    this.pool.forEachActive((e) => {
      ...
      // 時光戒：範圍內的敵人另外標記一個獨立旗標（不共用 slowUntil），避免跟
      // frost 系的減速持續時間/冰凍累積互相干擾——每幀在範圍內就刷新一次，
      // 離開範圍後 200ms 內自然過期，緊貼著「範圍內才生效」的語意。
      if (this.scene._hasTimeRing && dist(e.x, e.y, px, py) <= TIME_RING_RADIUS) {
        e.setData('timeRingUntil', now + 200);
      }
      const timeSlowed = now < e.getData('timeRingUntil');
      ...
      if (frozen) {
        ...
        // 跟遊戲既有的減速規則一致：不管幾種減速來源同時生效，強度都固定卡在
        // 50%，不會疊加變更慢（見檔案開頭 SLOW_SPEED_FACTOR 的說明）。
        const slowed = (now < e.getData('slowUntil')) || timeSlowed ? SLOW_SPEED_FACTOR : 1;
      ...
      if (now >= e.getData('flashUntil')) {
        ...
        else if (now < e.getData('slowUntil') || timeSlowed) e.setTint(0xb8e4ff);
        else {
      ...
      // 接觸傷害：冰凍期間怪物完全動彈不得，也不會主動造成接觸傷害；時光戒範圍內
      // 額外把攻擊間隔拉長到兩倍（=攻速 -50%），跟移動減速用同一個 timeSlowed 判斷。
      const atkInterval = timeSlowed ? TIME_RING_ATK_INTERVAL : 500;
      if (!frozen && dist(e.x, e.y, px, py) < 20 && now - e.getData('lastHitAt') > atkInterval) {
  ...
  // skipRingMult：連鎖戒觸發的連鎖傷害呼叫這裡時會傳 true，避免狂怒戒/蓄力戒的
  // 傷害乘數在同一次觸發裡被算兩次（連鎖傷害本身已經是「乘算後」的數值，見
  // GameScene.triggerChainRing 怎麼呼叫）。
  damageEnemy(enemy, baseDmg, critRate = 0, critDmg = 150, knockback = null, skipRingMult = false) {
    let dmg = baseDmg;
    let isCrit = false;
    ...
    if (!skipRingMult) dmg *= this.scene.getRingDamageMultiplier();
    ...
    // 連鎖戒：額外對場上一名隨機敵人補上同傷害的連鎖攻擊（見 GameScene.triggerChainRing）
    if (!skipRingMult) this.scene.triggerChainRing(dmg, enemy);
  ...
  // 輪迴戒用：只清空指定圓心半徑內的小怪（魔王不在 pool 裡，天生就不受影響），
  // 跟 killAllActive() 一樣走正常擊殺流程（掉經驗寶石、播特效、計入擊殺數）。
  killActiveInRadius(x, y, radius) {
    for (const enemy of Array.from(this.pool.active)) {
      if (dist(x, y, enemy.x, enemy.y) <= radius) this._killEnemy(enemy);
    }
  }
}
