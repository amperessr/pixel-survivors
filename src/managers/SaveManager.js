...
// 卻判定「背包已滿」（因為真正的存檔陣列只有 50 格，超出的視覺空格根本不存在）。
...
// ---------- 新手禮包：帳號第一次建立時才會標記，見 promptPlayerName 的新帳號分支 ----------
// isNewbie 帳號才看得到 MailboxScene 動態組出的「新手禮包」信（不寫進 MailData.js
// 固定清單，避免舊玩家也看到）；同一時間會把當下 MailData.js 已存在的信全部標成
// 已刪除，讓新玩家看不到「這個帳號建立之前就發過」的舊信，之後才發的新信不受影響。
export function isNewbieAccount() {
  return localStorage.getItem(NEWBIE_KEY) === '1';
}

function _markAsNewbieAccount() {
  localStorage.setItem(NEWBIE_KEY, '1');
  const status = getMailStatus();
  MAIL_DATA.forEach((m) => { status[m.id] = 'deleted'; });
  localStorage.setItem(MAIL_STATUS_KEY, JSON.stringify(status));
}
...
// ---------- 扭蛋保底：連續 100 抽沒出傳說（或更高）以上就強制出一件（見 ShopScene._gachaPull）----------
export function getGachaPity() {
  return Math.max(0, parseInt(localStorage.getItem(GACHA_PITY_KEY) || '0', 10));
}

export function setGachaPity(n) {
  localStorage.setItem(GACHA_PITY_KEY, String(Math.max(0, Math.floor(n))));
  _scheduleCloudPush();
}
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      if (exists) {
        ...
        // 新帳號：標記成新手（讓 MailboxScene 顯示新手禮包、隱藏這個時間點以前
        // 發過的舊信，見 _markAsNewbieAccount），再把這台裝置目前的存檔內容
        // 當作初始進度存上雲端
        _markAsNewbieAccount();
